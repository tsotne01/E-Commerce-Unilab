      ...
      <div className="flex md:max-w-[1440px] mx-auto gap-5 md:justify-center w-full">
        ...
        <section className="md:w-[75%] w-full">
          <div className="flex justify-between w-full md:w-full">
            ...
            <div>
              <span className="text-[#00000099]">Showing 1-10 of 100 Products</span>
            </div>
          </div>
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 px-5 py-4 gap-x-5 gap-y-9">
            {categoryItems && categoryItems.map((item) => <div className="max-w-2xs mx-auto sm:mx-0"><Link to={`/product-details/${item.id}`}><ItemCard key={item.id} item={Object.assign({}, item, { image: shirtImage })} /></Link></div>
            )}
          </div>
        </section>
      </div>
