    ...
    <main className="max-w-[1440px] mx-auto">
      <div className="ml-6 pt-7 border-[#0000001A] mb-6 border-t-1 satoshi text-base flex items-center gap-x-3">
        <div className="flex gap-x-1">
          ...
          <img className="inline-block" src={arrowRightGray} alt="arrow right" />
        </div>
        <span className="text-black">Cart</span>
      </div>
      <div>
        <h2 className="integral-cf mb-6 font-extrabold text-[40px]">Your cart</h2>
        <div className="flex gap-x-5  ">
        {!cartItems && <span>Cart is empty !</span>}

          <div className=" rounded-[20px] px-5 py-6 border-1 border-[#0000001A]">
            {cartItems && cartItems.map((item)=>{
              return (<div key={item.id} className="border-b-1 mb-6 border-[#0000001A] min-w-3xl justify-between flex">
              <div className="flex gap-x-3 mb-6">
                <img src={shirtImage} className="w-32 h-32 rounded-xl" alt="shirt image" />
                <div>
                  <h3 className="satoshi text-[20px] mb-0.5">{item.name}</h3>
                  <span className="block mb-1">Size: {item.sizes[0]}</span>
                  <span className="block mb-3.5">Color: {item.colors[0]}</span>
                  <span className="block satoshi text-2xl font-bold">${item.price}</span>
                </div>
              </div>
